...
## `axapta-mcp-helper` не дублирует поведение сервера

Решение 13.09.2026: то, какой MCP-инструмент axapta-mcp-server как себя
ведёт (требует ли живого поллера, что означает `origin` в ответе, что
доступно без AX-клиента) — описывает сам сервер (`[Description]`
инструментов + `mcp_guide(audience)`), не этот скилл. Раньше здесь была
отдельная таблица классификации инструментов — она разошлась с реальностью
после того, как сервер перевёл 5 инструментов на mirror/SQL-фоллбэк (раздел
C плана PollerAccess), а скилл остался старым.

Если при правке `axapta-mcp-helper` видишь описание того, что делает
конкретный инструмент без поллера/что значит `origin` — это сигнал вычистить
такой кусок и заменить отсылкой на `mcp_guide(audience="ax2012-dev")`/
`mcp_guide(audience="mirror-consumer")`, а не подгонять текст под текущую
версию сервера. Оставлять в скилле — только то, что сервер знать не может:
IDE-специфику (Claude Code/Cursor конфиг) и ALK-специфичный процесс
(проектный реестр `CIT_ProjectJobTable`, мод-комментарии).
