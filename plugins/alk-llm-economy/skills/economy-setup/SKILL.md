...
- `model` и `effortLevel` — фрагмент ставит дефолт команды `sonnet` + `medium`
  (выше `medium` Sonnet всё равно не поднимется: `modelSettings` ограничивает
  его для исполнителей). Если у владельца другая модель по умолчанию — убрать
  эти ключи из результата слияния или поправить после; хук `model-notice`
  будет раз за сессию напоминать об отличии от дефолта команды.
