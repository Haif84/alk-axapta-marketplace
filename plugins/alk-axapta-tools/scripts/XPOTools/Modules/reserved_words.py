...
RESERVED_WORDS = frozenset(w.lower() for w in (
    ...
    "sessionid",
    # Ключевые слова запросов, которых нет в выгруженном списке Axapta: в select
    # они разбираются как ключевые слова, но как имя переменной компилятор их,
    # судя по списку, пропускает. Запрещены здесь сознательно — переменная с
    # именем where или group читается как часть запроса и сбивает при чтении.
    # Проверено, что запрет ничего существующего не ломает: ни в боевом AOT
    # (Classes и Forms — самые крупные контейнеры), ни в прикладной выгрузке на
    # 31 тысячу строк X++ объявлений с такими именами нет ни одного.
    "where", "group", "having",
    # boolean — примитивный тип; в список Axapta не попал по той же причине.
    "boolean",
))
