...
См. [setup](../setup/SKILL.md) §«Preflight-конвенция». Прогони перед `organize`/`flatten`/`sync`/`cleanup`:
...
`bin/` **не** добавляется в PATH автоматически (известное ограничение — Claude Code не
модифицирует Windows PATH при установке плагина); если короткие команды не находятся —
используй Fallback ниже с полным путём через `${CLAUDE_PLUGIN_ROOT}`:
