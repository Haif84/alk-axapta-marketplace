...
- **Плоский layout** (legacy): `xpo\Class_Foo.xpo`, `xpo\Table_Bar.xpo`, `xpo\MenuItemDisplay_Baz.xpo`. Префикс типа в имени файла. Просто, но при 40+ файлах теряется навигация и пропадает прямая сопоставимость с боевой выгрузкой.
- **AOT layout** (предпочтительный): `xpo\Classes\Foo.xpo`, `xpo\Data Dictionary\Tables\Bar.xpo`, `xpo\Menu Items\Display\Baz.xpo`. Имена и пути совпадают с боевой выгрузкой `E:\ZeroCoder\Axapta\ERP\AOT-Prod\`, поэтому `diff` показывает реальные модификации без шума от различий в layout.
...
Все операции — Python-скрипты в `XPOTools/` (общий инструментарий ALK, бандлирован в плагин — `${CLAUDE_PLUGIN_ROOT}/scripts/XPOTools/`, документация — `${CLAUDE_PLUGIN_ROOT}/scripts/XPOTools/README.md`).

Плагин автоматически добавляет `bin/` в PATH, поэтому команды доступны сразу:
...
**Fallback при не настроенном PATH** — полные пути через переменную плагина:
```powershell
python "${CLAUDE_PLUGIN_ROOT}/scripts/XPOTools/organize-xpo.py" organize --root .\xpo --dry-run
python "${CLAUDE_PLUGIN_ROOT}/scripts/XPOTools/sync-xpo.py" --task-root .\xpo
python "${CLAUDE_PLUGIN_ROOT}/scripts/XPOTools/cleanup-xpo.py" --root .\xpo --empty-dirs
```

## Источник маппинга

Маппинг «префикс файла → тип объекта → AOT-подпапка» зафиксирован в `${CLAUDE_PLUGIN_ROOT}/scripts/XPOTools/Modules/xpo_types.py`:
...
- **Источник маппинга**: `${CLAUDE_PLUGIN_ROOT}/scripts/XPOTools/Modules/xpo_types.py`.
...
- **`axapta-project-export`** — финальная сборка бандла. **Рекомендуется** прогонять `organize-xpo organize` + `validate-xpo --strict` ПЕРЕД сборкой. `build-shared-project` поддерживает оба layout (плоский и AOT) — рекурсивный glob был добавлен в рамках этой инфраструктуры.
