...
### Проекты с нестандартным layout исходников

Папка `XPO/` в корне задачи — конвенция **по умолчанию** для типичных ALK-тикетов
(`LT_DAX-NNNN\XPO\`), а не жёсткое требование инструмента. Для репозиториев другого
типа (например, разработка самого тулинга, где X++ исходники лежат в `src/xpp/Classes/`,
`src/xpp/Jobs/` и т.п.) — указывай эту папку через `--root` у `build-shared-project`
и остальных команд `XPOTools`. Сканирование рекурсивное (`Glob "**/*.xpo"`), поэтому
произвольный layout поддерживается без изменений в скрипте.
