...
description: Собирает финальный xpo-файл задачи в Microsoft Dynamics AX (объект AOT Project + тела всех его объектов в одном файле) по конвенции ALK. Используй при завершении работ по тикету или по явному запросу пользователя ("собери выгрузку", "делаем релиз", "упакуй проект", "закрываем тикет"). Также предложи (вопросом, не молча) промежуточный чекпоинт-релиз при завершении фазы/milestone в многофазных проектах ("фаза завершена", "спайк готов", "этап реализован") — не дожидаясь закрытия всего тикета. Папка XPO/ в корне И AOT-раскладка внутри (Classes/, Data Dictionary/Tables/, …) ОБЯЗАТЕЛЬНЫ для любого проекта с Axapta-модификациями — если структура не такая, скилл приводит к ней (для нового проекта — пустая XPO/; перенос из src/ и раскладку плоских .xpo по типам делает только с подтверждения, не молча). Формирует имя SharedProject_<AOTProjectName>_<YYYYMMDD>_<HHMM>.xpo, проверяет наличие всех объектов, размещает файл в XPO/_release/. Перед сборкой запрашивает подтверждение имени AOT Project (по умолчанию <ProjectId>_<ModCode>_<UserNick> из ENV/активной memory) и списка включаемых объектов.
---
...
Перед релизом, если в задаче есть **новый/изменённый Excel Resource** или шаблон `XMLExcelReport_RU` — сначала скилл **`axapta-xpo-helper`** (упаковка Resource + конвенции Header/Body/дат), затем этот export.
...
Обязательна не только сама папка `XPO/`, но и **AOT-раскладка внутри неё** — объекты разложены
по подпапкам по типу (`XPO/Classes/`, `XPO/Data Dictionary/Tables/`, `XPO/Menu Items/Display/`,
`XPO/Jobs/`, …), как в боевой выгрузке `AOT-Prod`. Плоская куча `.xpo` в корне `XPO/` — это
промежуточное состояние, а не конечное: её приводят к AOT-структуре через `organize-xpo`
(скилл `axapta-project-manage`). Технически `build-shared-project` собирает и из плоского layout
(рекурсивный glob), но **канонической** считается AOT-раскладка — на неё рассчитаны `sync-xpo`
(diff с `AOT-Prod`), навигация и композиция инструментов между собой.

**Если структура не соответствует конечной** (`XPO/` в корне + AOT-подпапки) — привести к ней:
1. `.xpo`-файлы лежат вне `XPO/` (например, разбросаны по `src/`), либо папки `XPO/` нет —
   **спроси пользователя** через `AskUserQuestion`, перенести ли их в `XPO/`: перемещение меняет
   структуру репозитория, поэтому делается только с подтверждения, не молча.
2. `.xpo` уже в `XPO/`, но лежат плоско (без AOT-подпапок) — **спроси**, разложить ли их по типам
   через `organize-xpo organize --root .\XPO` (сначала `--dry-run`). Это и есть приведение к
   конечной структуре.
3. Проект только начинается (файлов ещё нет) — создать пустую `XPO/` сразу при первом же
   `.xpo`-файле, до вызова `axapta-mod-comments` (ничего не переносится — подтверждение не нужно).
4. Сообщить пользователю, что папка создана / файлы перенесены / разложены, одним коротким абзацем.

## Исполнитель сборки

**Основной путь** — Python-скрипт `XPOTools/build-shared-project.py` (бандлирован в плагин —
`"$pluginRoot\scripts\XPOTools\"`, см. [setup](../setup/SKILL.md) §«Resolve plugin root»;
документация — `"$pluginRoot\scripts\XPOTools\README.md"`). `bin/` **не** добавляется в PATH
автоматически; если короткие команды не находятся — Fallback через `$pluginRoot`:
...
```powershell
python "$pluginRoot\scripts\XPOTools\build-shared-project.py" --root .\XPO ...
```
...
### 0. Preflight ENV-гейт (ПЕРВЫЙ ШАГ — до любых инструментов)

Разреши `$pluginRoot` и прогони preflight (см. [setup](../setup/SKILL.md)
§«Resolve plugin root» и §«Preflight-конвенция»):
...
0. **Проверь layout XPO/** (см. §«Папка `XPO/` обязательна»). Конечная структура — AOT-раскладка по подпапкам (`XPO/Classes/`, `XPO/Data Dictionary/Tables/`, …). Если файлы лежат плоско (`XPO\Class_*.xpo`, `XPO\Table_*.xpo`, ...) — это промежуточное состояние: **спроси пользователя** и прогони `organize-xpo organize --root .\XPO` (сначала `--dry-run`), скилл [axapta-project-manage](file:///C:/Users/akaz/.claude/skills/axapta-project-manage/SKILL.md). Технически `build-shared-project` соберёт и из плоского layout (рекурсивный glob), но приведение к AOT — обязательный шаг перед финальным релизом: на AOT-раскладку рассчитаны `sync-xpo` (diff с `AOT-Prod`) и навигация. `validate-xpo --strict` теперь тоже это ловит: плоский корень даёт `WARN` (валит сборку только под `--strict` — не ломает уже идущие задачи без флага), а файл не в той AOT-подпапке для своего типа — `ERROR` (всегда, обычно значит, что `organize-xpo` ошибся).
...
**Рекурсивный** `Glob "**/*.xpo"` в `XPO/` папки задачи (технически берёт оба layout — плоский корень `XPO/Class_*.xpo` и AOT-структуру `XPO/Classes/*.xpo`, `XPO/Data Dictionary/Tables/*.xpo`, ...; канонической и обязательной перед релизом считается AOT-раскладка — см. §«Папка `XPO/` обязательна»). Для каждого файла:
...
```powershell
build-shared-project --root <project>\XPO --project-name ALK_DEVAX12_<TICKET>_<NICK> --dry-run
```
...
- **Не зашивай локальные пути в код** (например `E:\ZeroCoder\LT_*\XPO`). Все ALK-специфичные значения — через ENV-переменные `AX_PROJECT_ID`/`AX_USER_NICK`/`AX_AOT_PATH` (задаются один раз через скилл `/alk-axapta-tools:setup`).
