...
2. **Путь к AOT-Prod** (`AX_AOT_PATH`) — опционально. Нужен только для `sync-xpo`
   (сверка с офлайн-выгрузкой); при работе через живой MCP-сервер AX можно пропустить
   и задать позже. `sync-xpo` без него завершится понятной ошибкой, остальные
   инструменты не пострадают.
