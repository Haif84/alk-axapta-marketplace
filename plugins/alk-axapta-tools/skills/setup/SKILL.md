...
соседнем репозитории, где модификация уже другая. Из них собирается мод-маркер
целиком, а `AX_AOT_PROJECT` даёт имя AOT-проекта для сборки релиза:

```
//{AX_PROJECT_ID}, {AX_MODIFICATION_ID}, {AX_MODIFICATION_DESC}, {дата}, {AX_USER_NICK}
```

Вывести `AX_AOT_PROJECT` из `AX_PROJECT_ID` нельзя: смысловая часть имени и ник в
нём произвольные, поэтому имя хранится как есть. Готовый маркер отдаёт
...
5. **Код модификации** (`AX_MODIFICATION_ID`) — например `MOD-240-APP`.
6. **Описание модификации** (`AX_MODIFICATION_DESC`) — то, что попадёт в маркер
   дословно, например `Разработка MCP-сервера для Axapta (by AKAZ)`.
7. **Имя AOT-проекта** (`AX_AOT_PROJECT`) — например
   `ALK_CDT000_MOD_240_APP_MCPServer_akaz`. Предложи по умолчанию
   `{AX_PROJECT_ID}_{код модификации через подчёркивания}_{смысловая часть}_{ник}`,
   но обязательно **дай подтвердить**: смысловая часть произвольная, а порядок
   элементов в разных модификациях отличается.

Зачем спрашивать все три сразу: из них собирается мод-маркер целиком, и без них
`axapta-mod-comments` будет выпрашивать те же значения в начале каждой сессии.
...
Блок `[задача — из .axapta.json]` должен показать все три значения.
